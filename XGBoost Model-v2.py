...
# - Smart hyperparameter tuning (Optuna if available; fallback to RandomizedSearchCV)
...
if not used_optuna:
    ...
    param_dist = {
        ...
        'learning_rate': uniform(1e-3, 0.3-1e-3),
        'subsample': uniform(0.5, 0.5),
        'colsample_bytree': uniform(0.5, 0.5),
        'min_child_weight': randint(1, 15),
        'gamma': uniform(0.0, 5.0),
        'reg_lambda': uniform(1e-3, 50.0),
        'reg_alpha': uniform(1e-3, 10.0),
    }
...
best_estimator = xgb.XGBRegressor(**{**base_params(), **best_params})
...
try:
    import shap
    explainer = shap.Explainer(best_estimator, X_train, feature_names=FEATURES)
...
cv_results.to_excel(OUTDIR / 'xgb_10fold_predictions.xlsx', index=False)
